...
if st.session_state["login"] == "notloggedin":
    ...
    if loan_selection == "Loan a book":       
        book_df = st.dataframe(display_books(), selection_mode="single-row", on_select="rerun", key="loan_book_df")
        if len(book_df.selection["rows"]) > 0:
            book = read_books().loc[book_df.selection["rows"][0]]
            val1 = validate_loan_item(book)
            if val1:
                st.warning(val1)
        friend_df = st.dataframe(display_friends(), selection_mode="single-row", on_select="rerun", key="loan_friend_df")
        ...
        st.dataframe(display_loans(), key="display_loans_df")
    
    elif loan_selection == "Update a loan":
        loan_df = st.dataframe(display_loans(), selection_mode="single-row", on_select="rerun", key="update_loan_df")
        ...
        loan_df = st.dataframe(display_loans(), selection_mode="single-row", on_select="rerun", key="delete_loan_df")
        if len(loan_df.selection["rows"]) > 0:
            loan = read_loans().loc[loan_df.selection["rows"][0]]
    ...
    if book_selection == "Add a book":
        ...
        st.dataframe(display_books(only_available), key="display_books_df")
    
    elif book_selection == "Update a book":
        book_df = st.dataframe(display_books(), selection_mode="single-row", on_select="rerun", key="update_book_df")
        if len(book_df.selection["rows"]) > 0:
            book = read_books().loc[book_df.selection["rows"][0]]
        ...
        book_df = st.dataframe(display_books(), selection_mode="single-row", on_select="rerun", key="delete_book_df")
        if len(book_df.selection["rows"]) > 0:
            book = read_books().loc[book_df.selection["rows"][0]]
    ...
    if friend_selection == "Review friends":
        st.dataframe(display_friends(), key="display_friends_df")
        ...
        friend_df = st.dataframe(display_friends(), selection_mode="single-row", on_select="rerun", key="update_friend_df")
        if len(friend_df.selection["rows"]) > 0:
            friend = read_friends().loc[friend_df.selection["rows"][0]]
        ...
        friend_df = st.dataframe(display_friends(), selection_mode="single-row", on_select="rerun", key="delete_friend_df")
        if len(friend_df.selection["rows"]) > 0:
            friend = read_friends().loc[friend_df.selection["rows"][0]]
